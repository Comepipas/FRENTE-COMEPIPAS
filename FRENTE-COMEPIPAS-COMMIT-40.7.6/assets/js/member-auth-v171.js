...
async function activate({firstName,lastName,email,password}){const c=await client();const clean={firstName:String(firstName||'').trim(),lastName:String(lastName||'').trim(),email:String(email||'').trim().toLowerCase()};if(!clean.firstName||!clean.lastName||!clean.email)throw new Error('Debes indicar nombre, apellidos y correo electrónico.');const {data:check,error:checkError}=await c.rpc('check_member_activation_identity',{p_nombre:clean.firstName,p_apellidos:clean.lastName,p_email:clean.email});if(checkError)throw checkError;if(!check?.ok)throw new Error(check?.message||'Los datos no coinciden con una ficha activa del censo.');const options={emailRedirectTo:redirect('confirmacion-cuenta.html'),data:{member_first_name:clean.firstName,member_last_name:clean.lastName,member_email:clean.email}};const {data,error}=await c.auth.signUp({email:clean.email,password,options});if(error)throw error;if(data.user&&!data.session)return{confirmationRequired:true};await completeLink();return{confirmationRequired:false,profile:await profile()};}
