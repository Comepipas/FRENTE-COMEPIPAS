...
const TEAM_LOGOS={
  "Málaga CF":"assets/images/teams/pro/malaga-cf.png",
  "Atlético de Madrid":"assets/images/teams/pro/atletico-de-madrid.png",
  "RC Deportivo":"assets/images/teams/pro/rc-deportivo.png",
  "Real Madrid":"assets/images/teams/pro/real-madrid.png"
};
...
function teamLine(team){return `<div class="v387-team"><img class="v387-crest" src="${esc(teamLogo(team))}" alt="Escudo de ${esc(team)}" loading="lazy"><span>${esc(team)}</span></div>`}
...
function renderMatches(){const box=document.querySelector("#v27NextMatches");if(!box)return;const rows=nextRows();box.innerHTML=rows.map(m=>{const local=isMalaga(m.home);return `<article class="v387-match"><img class="v387-stadium" src="${esc(stadiumImage(m.stadium))}" alt="${esc(m.stadium||"Estadio")}" loading="lazy"><div class="v387-photo-fade"></div><div class="v387-card-body"><span class="v387-round">${esc(m.competition||"Partido")}</span><div class="v387-fixture">${teamLine(m.home)}<span class="v387-vs">VS</span>${teamLine(m.away)}</div><div class="v387-details"><p><span aria-hidden="true">📅</span>${esc(fmt(m))}</p><p><span aria-hidden="true">🏟️</span>${esc(m.stadium||"Estadio por confirmar")}</p><span class="v387-status">${local?"🏠 LOCAL":"✈️ VISITANTE"}</span></div></div></article>`}).join("")||'<div class="v27-home-empty">No hay partidos configurados.</div>'}
