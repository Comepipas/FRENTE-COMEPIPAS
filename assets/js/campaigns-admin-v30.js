...
async function saveRecord(e){e.preventDefault();const o=Object.fromEntries(new FormData(e.target));const id=o.id;delete o.id;for(const k of ['precio_abono','descuento_club','cuota_final','importe_pagado'])o[k]=Number(o[k]||0);o.es_directivo=o.es_directivo==='true';if(!o.forma_pago)o.forma_pago=null;if(o.estado==='pagado'&&!o.fecha_pago)o.fecha_pago=new Date().toISOString();const {error}=await sb.from('campanas_registros').update(o).eq('id',id);if(error)return msg(error.message);$('#editRecordModal').classList.remove('open');await loadAll();}
