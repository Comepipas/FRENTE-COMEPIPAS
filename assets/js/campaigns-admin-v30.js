...
async function init(){try{sb=(await FrenteSupabase.init()).client;$('#c28Connection').textContent='Supabase conectado';$('#c28Connection').className='badge badge-success';bind();await loadCampaigns();openRequestedTab();}catch(e){msg(e.message);$('#c28Connection').textContent='Sin conexión';}}
function openRequestedTab(){const requested=new URLSearchParams(location.search).get('tab');if(!requested)return;const button=$(`.c28-tab[data-tab="${requested}"]`);if(button)button.click()}
function bind(){
 $('#c28Reload').onclick=()=>loadCampaigns(true); $('#campaignSelect').onchange=async e=>{current=campaigns.find(c=>c.id===e.target.value);await loadAll();};
 ...
 $('#issueForm').onsubmit=saveIssue; $('#closeIssueModal').onclick=()=>$('#issueModal').classList.remove('open'); $('#linkIssue').onclick=linkIssueRecord; $('#editIssueRecord').onclick=editIssueRecord;
...
async function loadCampaigns(force=false){const selectedId=current?.id||null;const query=sb.from('campanas_abonados').select('id,temporada,nombre,tipo,estado,fecha_corte,fecha_apertura,fecha_cierre,pago_online_activo,modo_pruebas,altas_post_cierre,texto_socio,configuracion,created_at,updated_at').order('temporada',{ascending:false}).order('updated_at',{ascending:false}).limit(1000);const {data,error}=await query;if(error)throw error;campaigns=data||[];current=selectedId&&campaigns.find(c=>c.id===selectedId)||campaigns[0]||null;$('#campaignSelect').innerHTML=campaigns.map(c=>`<option value="${c.id}" ${c.id===current?.id?'selected':''}>${esc(c.nombre)}</option>`).join('');await loadAll();if(force)msg('Campañas y registros actualizados desde Supabase.');}
async function loadAll(){if(!current)return;const [a,b,c,d,e]=await Promise.all([sb.from('campanas_categorias').select('*').eq('campana_id',current.id).order('orden'),sb.from('campanas_registros').select('*,socios(numero_socio,nombre,apellidos,fecha_nacimiento)').eq('campana_id',current.id).order('created_at',{ascending:false}),sb.from('campanas_incidencias').select('*').eq('campana_id',current.id).order('created_at',{ascending:false}),sb.from('campanas_piloto_participantes').select('*,socios(numero_socio,nombre,apellidos)').eq('campana_id',current.id),sb.from('socios').select('id,numero_socio,nombre,apellidos,dni,fecha_nacimiento,estado,es_directivo')]);for(const r of [a,b,c,d,e])if(r.error)throw r.error;categories=a.data||[];records=b.data||[];issues=c.data||[];members=(e.data||[]).filter(x=>x.estado!=='baja');renderCampaign();renderCategories();renderRecords();renderIssues();renderPilot(d.data||[]);runDiagnosis(false);runSimulation(false);}
...
function categoryMemberCount(c){if(String(c.nombre).toLowerCase()==='directivo')return members.filter(m=>m.es_directivo).length;if(!c.nacimiento_desde||!c.nacimiento_hasta)return 0;return members.filter(m=>m.fecha_nacimiento&&m.fecha_nacimiento>=c.nacimiento_desde&&m.fecha_nacimiento<=c.nacimiento_hasta).length;}
function renderCategories(){
 const missing=categories.filter(c=>c.activa&&String(c.nombre).toLowerCase()!=='directivo'&&(!c.nacimiento_desde||!c.nacimiento_hasta)).length;
 const overlaps=categories.filter((c,i)=>c.activa&&String(c.nombre).toLowerCase()!=='directivo'&&c.nacimiento_desde&&c.nacimiento_hasta&&categories.some((o,j)=>j!==i&&o.activa&&String(o.nombre).toLowerCase()!=='directivo'&&o.nacimiento_desde&&o.nacimiento_hasta&&c.nacimiento_desde<=o.nacimiento_hasta&&c.nacimiento_hasta>=o.nacimiento_desde)).length;
...
function memberFor(item,list){const all=list||[],candidates=all.filter(m=>normalizeKey(m.dni)===item.key),wanted=normalizeKey(item.name),dniAndName=candidates.find(m=>normalizeKey(`${m.nombre||''} ${m.apellidos||''}`)===wanted);if(dniAndName)return dniAndName;const byName=wanted?all.filter(m=>normalizeKey(`${m.nombre||''} ${m.apellidos||''}`)===wanted):[];if(byName.length===1)return byName[0];return !wanted&&candidates.length===1?candidates[0]:null}
...
async function syncUnlinkedCampaignRows(rows){
 if(!rows.length)return {updated:0,inserted:0,removed:0};
 const currentRows=await sb.from('campanas_registros').select('id,dni_club,nombre_club,created_at').eq('campana_id',current.id).is('socio_id',null).order('created_at',{ascending:false});if(currentRows.error)throw currentRows.error;
 const keyOf=r=>`${normalizeKey(r.dni_club)}|${normalizeKey(r.nombre_club)}`,groups=new Map();(currentRows.data||[]).forEach(r=>{const key=keyOf(r);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(r)});
 const fresh=[],obsolete=[];let updated=0;
 for(const row of rows){const matches=groups.get(keyOf(row))||[];if(matches.length){const keep=matches.shift(),result=await sb.from('campanas_registros').update(row).eq('id',keep.id);if(result.error)throw result.error;updated++;obsolete.push(...matches.map(x=>x.id));groups.set(keyOf(row),[])}else fresh.push(row)}
 if(fresh.length)await insertInChunks('campanas_registros',fresh);
 if(obsolete.length){const removed=await sb.from('campanas_registros').delete().in('id',obsolete);if(removed.error)throw removed.error;}
 return {updated,inserted:fresh.length,removed:obsolete.length};
}
async function confirmImport(){
 ...
 try{
  ...
  const origin=$('#importOrigin').value,matched=[],unmatched=[],newIssues=[],claimedMembers=new Map();
  for(const item of previewAnalysis.valid){
   ...
   const alreadyClaimed=member&&claimedMembers.has(member.id);
   const safeMember=alreadyClaimed?null:member;
   const row={campana_id:current.id,socio_id:safeMember?.id||null,origen:origin,dni_club:item.dni||null,nombre_club:item.name||null,zona_club:pick(r,['nombre zona del abono','zona','sector']),sector_club:pick(r,['id. zona abono','id zona abono','número zona','numero zona','sector id']),categoria_club:pick(r,['tipo de abono','tipo abono','categoria','categoría']),precio_original:original,descuento_club:discount,precio_abono:price,categoria_pena:pick(r,['categoria pena','categoría peña']),cuota_base:fee,cuota_final:fee,gestion_abono:gestion,estado:check.ok&&safeMember?'pagado':'incidencia',forma_pago:check.ok?'transferencia':null,importe_pagado:paid,fecha_pago:check.ok?(paymentDate||new Date().toISOString()):null,datos_origen:r,observaciones:alreadyClaimed?'Sin vincular automáticamente: otra fila del Excel coincide con el mismo socio interno.':(check.ok?'Conciliado automáticamente: DESCUADRE = 0 en el Excel del club.':`Pendiente de revisión: DESCUADRE ${String(check.raw??'vacío')}.`)};
   if(safeMember){claimedMembers.set(safeMember.id,item);matched.push(row);}else unmatched.push(row);
   if(alreadyClaimed){const first=claimedMembers.get(member.id);newIssues.push({campana_id:current.id,tipo:'vinculacion_ambigua',gravedad:'alta',descripcion:`No se vinculó automáticamente ${item.name||item.dni}: coincide con el mismo socio interno que ${first?.name||'otra fila del Excel'}. Revisar DNI compartido o identidad familiar.`});}
   else if(!member)newIssues.push({campana_id:current.id,tipo:'socio_no_localizado',gravedad:'media',descripcion:`Pendiente de vincular ${item.name||item.dni} con su ficha. No es una incidencia de pago ni de número de socio.`});
  ...
  const unlinkedSync=await syncUnlinkedCampaignRows(unmatched);
  const issueKeys=new Set(issues.map(i=>`${i.tipo}|${normalizeKey(i.descripcion)}`)),uniqueIssues=newIssues.filter(i=>{const key=`${i.tipo}|${normalizeKey(i.descripcion)}`;if(issueKeys.has(key))return false;issueKeys.add(key);return true});
  if(uniqueIssues.length)await insertInChunks('campanas_incidencias',uniqueIssues);
  const matchedNames=matched.map(r=>normalizeKey(r.nombre_club)).filter(Boolean),resolvedLinks=issues.filter(i=>i.estado==='abierta'&&isLinkIssue(i)&&matchedNames.some(name=>normalizeKey(i.descripcion).includes(name))).map(i=>i.id);if(resolvedLinks.length)await sb.from('campanas_incidencias').update({estado:'resuelta',resolucion:'Vinculación reconocida durante la resincronización del Excel 2026/27.',resolved_at:new Date().toISOString()}).in('id',resolvedLinks);
  const created=mode==='create_members'?previewAnalysis.newMembers.length:0;
  importMsg(`Sincronización completada: ${matched.length+unmatched.length} registros 2026/27, ${created} socios nuevos, ${unmatched.length} pendientes de vincular, ${uniqueIssues.length} incidencias nuevas y ${unlinkedSync.removed} duplicados técnicos eliminados.`);
...
function paymentView(r){const paid=Number(r.importe_pagado||0),expected=(r.gestion_abono==='pena'?Number(r.precio_abono||0):0)+Number(r.cuota_final||0),balanced=Math.abs(paid-expected)<.01;if(r.estado==='pagado'||(r.forma_pago&&balanced))return ['Pagado','ok'];if(paid>0)return ['Importe para revisar','bad'];return ['Pago pendiente','bad']}
function renderRecords(){const q=normalizeKey($('#recordSearch').value);const list=records.filter(r=>!q||normalizeKey(`${r.socios?.numero_socio} ${r.socios?.nombre} ${r.socios?.apellidos} ${r.nombre_club}`).includes(q));$('#recordsBody').innerHTML=list.map(r=>{const payment=paymentView(r),linked=Boolean(r.socio_id&&r.socios);return `<tr><td>${esc(r.socios?`${r.socios.numero_socio||''} ${r.socios.nombre||''} ${r.socios.apellidos||''}`:r.nombre_club||'No vinculado')}</td><td>${esc(r.gestion_abono)}</td><td>${euro(r.precio_abono)}</td><td>${euro(r.cuota_final)}${r.es_directivo?' <span class="c28-badge ok">Directivo</span>':''}</td><td>${euro(r.importe_total)}</td><td>${euro(r.importe_pagado)}</td><td><span class="c28-badge ${payment[1]}">${payment[0]}</span></td><td><span class="c28-badge ${linked?'ok':'test'}">${linked?'Vinculado':'Pendiente de vincular'}</span></td><td><button class="btn btn-secondary" data-edit="${r.id}">Editar</button></td></tr>`}).join('');$$('[data-edit]').forEach(b=>b.onclick=()=>openRecord(b.dataset.edit));}
function ensureRecordMemberField(f){if(!f.elements.socio_id){const label=document.createElement('label');label.className='full';label.innerHTML='<span>Ficha del socio vinculada</span><select name="socio_id"></select><small>Selecciona la persona correcta. No es necesario que tenga número de socio definitivo.</small>';f.insertBefore(label,f.firstElementChild?.nextSibling||f.firstChild)}const stateLabel=f.elements.estado?.closest('label');if(stateLabel&&!stateLabel.dataset.economic){stateLabel.dataset.economic='true';stateLabel.childNodes[0].textContent='Estado económico'}}
function openRecord(id){const r=records.find(x=>x.id===id),f=$('#recordForm');ensureRecordMemberField(f);f.elements.socio_id.innerHTML='<option value="">Pendiente de vincular</option>'+members.slice().sort((a,b)=>`${a.nombre} ${a.apellidos}`.localeCompare(`${b.nombre} ${b.apellidos}`,'es')).map(m=>`<option value="${m.id}">${esc(`${m.nombre||''} ${m.apellidos||''}`.trim())} · ${esc(m.numero_socio||'n.º pendiente')} · ${esc(m.dni||'sin DNI')}</option>`).join('');for(const k of ['id','gestion_abono','estado','precio_abono','descuento_club','cuota_final','importe_pagado','forma_pago','observaciones'])f[k].value=r[k]??'';if(r.estado==='incidencia')f.elements.estado.value=paymentView(r)[0]==='Pagado'?'pagado':'pendiente_pago';f.elements.socio_id.value=r.socio_id||'';f.es_directivo.value=String(r.es_directivo);$('#editRecordModal').classList.add('open');}
async function saveRecord(e){e.preventDefault();const o=Object.fromEntries(new FormData(e.target)),id=o.id,record=records.find(r=>r.id===id);delete o.id;for(const k of ['precio_abono','descuento_club','cuota_final','importe_pagado'])o[k]=Number(o[k]||0);o.es_directivo=o.es_directivo==='true';o.socio_id=o.socio_id||null;if(!o.forma_pago)o.forma_pago=null;if(o.estado==='pagado'&&!o.fecha_pago)o.fecha_pago=new Date().toISOString();const {error}=await sb.from('campanas_registros').update(o).eq('id',id);if(error)return msg(error.message.includes('duplicate key')?'Ese socio ya tiene otro registro en esta campaña. Revisa primero la vinculación existente.':error.message);if(o.socio_id&&record?.nombre_club){const related=issues.filter(i=>i.estado==='abierta'&&isLinkIssue(i)&&normalizeKey(i.descripcion).includes(normalizeKey(record.nombre_club))).map(i=>i.id);if(related.length)await sb.from('campanas_incidencias').update({estado:'resuelta',resolucion:'Vinculado desde Abonados y cobros.',resolved_at:new Date().toISOString()}).in('id',related)}$('#editRecordModal').classList.remove('open');await loadAll();}
...
function isLinkIssue(i){return ['socio_no_localizado','vinculacion_ambigua'].includes(i?.tipo)}
function issueCard(i,link=false){return `<article class="c28-card"><span>${link?'PENDIENTE DE VINCULAR':esc(i.tipo)} · ${esc(i.estado)}</span><strong style="font-size:1rem">${esc(i.descripcion)}</strong>${i.resolucion?`<p><strong style="font-size:.9rem">Resolución:</strong> ${esc(i.resolucion)}</p>`:''}<div class="c28-actions"><button class="btn btn-secondary" data-open-issue="${i.id}">${link?'Revisar y vincular':'Abrir y resolver'}</button></div></article>`}
function renderIssues(){
 const open=issues.filter(i=>i.estado==='abierta'),payment=open.filter(i=>!isLinkIssue(i)),link=open.filter(isLinkIssue),closed=issues.filter(i=>i.estado!=='abierta');
 $('#issuesList').innerHTML=`<div class="c28-grid"><article class="c28-card"><span>Incidencias económicas</span><strong>${payment.length}</strong></article><article class="c28-card"><span>Pendientes de vincular</span><strong>${link.length}</strong></article><article class="c28-card"><span>Resueltas o descartadas</span><strong>${closed.length}</strong></article></div><div class="c28-alert"><strong>Importante:</strong> un socio pendiente de vincular no es una incidencia de pago ni significa que le falte el número definitivo. Solo falta relacionar la fila del Excel con su ficha correcta.</div><h3>Incidencias económicas abiertas</h3><div class="c407-issue-list">${payment.map(i=>issueCard(i)).join('')||'<p>No hay incidencias económicas abiertas.</p>'}</div><h3 style="margin-top:28px">Pendientes de vincular</h3><div class="c407-issue-list">${link.map(i=>issueCard(i,true)).join('')||'<p>No hay registros pendientes de vincular.</p>'}</div><details style="margin-top:28px"><summary><strong>Ver resueltas y descartadas (${closed.length})</strong></summary><div class="c407-issue-list">${closed.map(i=>issueCard(i,isLinkIssue(i))).join('')||'<p>No hay registros cerrados.</p>'}</div></details>`;
 $$('[data-open-issue]').forEach(b=>b.onclick=()=>openIssue(b.dataset.openIssue));
}
function recordForIssue(issue){const text=normalizeKey(issue?.descripcion);return records.filter(r=>r.nombre_club).sort((a,b)=>String(b.nombre_club).length-String(a.nombre_club).length).find(r=>text.includes(normalizeKey(r.nombre_club)))||null}
function openIssue(id){
 const issue=issues.find(i=>i.id===id);if(!issue)return;const f=$('#issueForm'),matched=recordForIssue(issue),link=isLinkIssue(issue);
 f.elements.id.value=issue.id;f.elements.estado.value=issue.estado;f.elements.gravedad.value=issue.gravedad;f.elements.resolucion.value=issue.resolucion||'';$('#issueDescription').textContent=issue.descripcion;$('#issueKind').textContent=link?'Pendiente de vincular':'Incidencia económica';
 $('#issueRecord').innerHTML='<option value="">Selecciona el registro del Excel</option>'+records.map(r=>`<option value="${r.id}" ${r.id===matched?.id?'selected':''}>${esc(r.nombre_club||r.socios?.nombre||'Sin nombre')} · ${esc(r.dni_club||'sin DNI')} · ${euro(Number(r.precio_abono||0)+Number(r.cuota_final||0))}</option>`).join('');
 $('#issueMember').innerHTML='<option value="">Selecciona el socio correcto</option>'+members.slice().sort((a,b)=>`${a.nombre} ${a.apellidos}`.localeCompare(`${b.nombre} ${b.apellidos}`,'es')).map(m=>`<option value="${m.id}">${esc(`${m.nombre||''} ${m.apellidos||''}`.trim())} · ${esc(m.numero_socio||'n.º pendiente')} · ${esc(m.dni||'sin DNI')}</option>`).join('');
 $('#issueLinkFields').hidden=!link;$('#linkIssue').hidden=!link;$('#editIssueRecord').hidden=!matched;$('#issueModal').classList.add('open');
}
async function saveIssue(e){e.preventDefault();const o=Object.fromEntries(new FormData(e.target)),id=o.id;delete o.id;delete o.record_id;delete o.member_id;o.resolved_at=o.estado==='abierta'?null:new Date().toISOString();const {error}=await sb.from('campanas_incidencias').update(o).eq('id',id);if(error)return msg(error.message);$('#issueModal').classList.remove('open');await loadAll();}
async function linkIssueRecord(){
 const f=$('#issueForm'),recordId=f.elements.record_id.value,memberId=f.elements.member_id.value;if(!recordId||!memberId)return msg('Selecciona el registro del Excel y el socio correcto.');
 const record=records.find(r=>r.id===recordId),isPaid=record?.forma_pago==='transferencia'&&Number(record?.importe_pagado||0)>0;
 const update={socio_id:memberId,estado:isPaid?'pagado':record?.estado,observaciones:`${record?.observaciones||''}\nVinculado manualmente desde revisión de incidencias.`.trim()};const linked=await sb.from('campanas_registros').update(update).eq('id',recordId);if(linked.error)return msg(linked.error.message);
 const closed=await sb.from('campanas_incidencias').update({estado:'resuelta',resolucion:f.elements.resolucion.value||'Registro vinculado manualmente con el socio correcto.',resolved_at:new Date().toISOString()}).eq('id',f.elements.id.value);if(closed.error)return msg(closed.error.message);$('#issueModal').classList.remove('open');msg('Registro vinculado y pendiente cerrado.');await loadAll();
}
function editIssueRecord(){const issue=issues.find(i=>i.id===$('#issueForm').elements.id.value),record=recordForIssue(issue);if(!record)return msg('No se encontró el registro económico relacionado.');$('#issueModal').classList.remove('open');openRecord(record.id)}
...
function calculationForRecord(r){
 ...
 const directivo=categories.find(c=>c.activa&&String(c.nombre).toLowerCase()==='directivo');
 const fee=r.es_directivo?Number(directivo?.cuota||0):Number(cat?.cuota||0);
...
function diagnosisData(){
 const cats=activeCategories(),missingRanges=categories.filter(c=>c.activa&&String(c.nombre).toLowerCase()!=='directivo'&&(!c.nacimiento_desde||!c.nacimiento_hasta));
