...
async function saveCategory(id){const o={};$$(`[data-cat="${id}"]`).forEach(i=>{o[i.dataset.field]=i.type==='checkbox'?i.checked:(i.value===''?null:(i.dataset.field==='cuota'?Number(i.value):i.value));});const directivo=String(o.nombre||'').toLowerCase()==='directivo';if(!directivo&&(!o.nacimiento_desde||!o.nacimiento_hasta))return msg('Debes indicar las dos fechas de nacimiento de la categoría.');if(!directivo&&o.nacimiento_desde>o.nacimiento_hasta)return msg('La fecha «Nacidos desde» no puede ser posterior a «Nacidos hasta».');const overlap=!directivo&&categories.some(c=>c.id!==id&&c.activa&&String(c.nombre).toLowerCase()!=='directivo'&&o.activa&&c.nacimiento_desde&&c.nacimiento_hasta&&o.nacimiento_desde<=c.nacimiento_hasta&&o.nacimiento_hasta>=c.nacimiento_desde);if(overlap)return msg('Este rango se solapa con otra categoría activa. Corrige las fechas antes de guardar.');const {error}=await sb.from('campanas_categorias').update(o).eq('id',id);if(error)return msg(error.message);if(current.tipo!=='piloto'&&!current.modo_pruebas){const sync=await sb.rpc('commit407_refresh_census_categories',{p_campaign:current.id});if(sync.error)return msg('Categoría guardada, pero falta ejecutar el SQL 040_7 para actualizar el censo.');msg(`Categoría guardada y censo recalculado (${sync.data?.updated||0} socios).`)}else msg('Categoría y fechas guardadas. La campaña piloto no modifica el censo real.');await loadAll();}
