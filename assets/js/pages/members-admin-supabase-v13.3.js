...
function renderTable(){const b=$("membersTableBody");b.innerHTML=state.rows.length?state.rows.map(m=>`<tr><td>${esc(m.numero)}</td><td><strong>${esc(m.nombreCompleto)}</strong><br><small>${esc(m.email||"")}</small><br><a class="member-record-link member-record-link-inline" href="ficha-socio-admin.html?id=${encodeURIComponent(m.id)}">Abrir ficha del socio</a></td><td>${pill(m.categoria,"category-pill")}</td><td>${pill(m.cuenta,m.cuenta_activada?"fee-ok":"fee-pending")}</td><td>${pill(m.estado,String(m.estado).toLowerCase()==="activo"?"status-active":"status-inactive")}</td><td>${pill(m.cuota,m.cuota_al_dia?"fee-ok":"fee-pending")}</td><td><strong>${esc(m.sector||"Sin asignar")}</strong><br><small>${money(m.precio_abono)}</small></td><td class="members-actions-cell"><a class="member-record-link" href="ficha-socio-admin.html?id=${encodeURIComponent(m.id)}">Abrir ficha</a><button data-edit="${esc(m.id)}">Edición rápida</button><button class="danger" data-delete="${esc(m.id)}">Baja</button></td></tr>`).join(""):'<tr><td colspan="8" class="members-empty">No hay socios para estos filtros.</td></tr>';b.querySelectorAll("tr").forEach((tr,i)=>{const m=state.rows[i];if(!m)return;tr.title="Pulsa para abrir la ficha del socio";tr.onclick=e=>{if(e.target.closest("button,a,input,select,label"))return;location.href=`ficha-socio-admin.html?id=${encodeURIComponent(m.id)}`};tr.ondblclick=e=>{if(!e.target.closest("button,a,input,select,label"))location.href=`ficha-socio-admin.html?id=${encodeURIComponent(m.id)}`}});b.querySelectorAll("[data-edit]").forEach(x=>x.onclick=()=>openForm(x.dataset.edit));b.querySelectorAll("[data-delete]").forEach(x=>x.onclick=()=>softDelete(x.dataset.delete))}
