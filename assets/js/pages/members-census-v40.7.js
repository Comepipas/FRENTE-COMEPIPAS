(()=>{
  ...
  const improveTable=()=>{
    ...
    const headings=table.querySelectorAll('thead th');if(headings[6]&&headings[6].textContent!=='Temporadas')headings[6].textContent='Temporadas';
  ...
  window.addEventListener('load',()=>{
    ...
    const observer=new MutationObserver(()=>{if(refreshPending)return;refreshPending=true;requestAnimationFrame(()=>{refreshPending=false;hideAutomaticFields();improveTable()})});observer.observe(document.body,{childList:true,subtree:true});
