(()=>{
  ...
  async function setupSeasonHistory(db){
    ...
    const fields='id,campana_id,estado,zona_club,sector_club,categoria_club,precio_original,descuento_club,precio_abono,cuota_final,importe_total,importe_pagado,gestion_abono,forma_pago,fecha_pago,pago_club_declarado_at,pago_club_verificado_at,observaciones,datos_origen,created_at';
    let result=await db.from('campanas_registros').select(fields).eq('socio_id',memberId).order('created_at',{ascending:false}).limit(30);
    let sectorAvailable=true;
    if(result.error&&/sector_club|schema cache|PGRST204/i.test(result.error.message||'')){sectorAvailable=false;result=await db.from('campanas_registros').select(fields.replace(',sector_club','')).eq('socio_id',memberId).order('created_at',{ascending:false}).limit(30)}
    if(result.error){box.innerHTML=`<div class="record-empty"><strong>No se pudo cargar el historial de abonos.</strong><br>${esc(result.error.message||'Error desconocido de Supabase')}</div>`;return}
    const campaignIds=[...new Set((result.data||[]).map(r=>r.campana_id).filter(Boolean))],campaignMap=new Map();
    if(campaignIds.length){const campaigns=await db.from('campanas_abonados').select('id,temporada,nombre,tipo,modo_pruebas').in('id',campaignIds);if(campaigns.error){box.innerHTML=`<div class="record-empty"><strong>No se pudieron relacionar las temporadas.</strong><br>${esc(campaigns.error.message)}</div>`;return}(campaigns.data||[]).forEach(c=>campaignMap.set(c.id,c))}
    const data=(result.data||[]).map(r=>({...r,sector_club:r.sector_club||r.datos_origen?.['Id. Zona Abono']||r.datos_origen?.['id. zona abono']||null,campanas:campaignMap.get(r.campana_id)||null}));
    ...
    if(!sectorAvailable)box.insertAdjacentHTML('afterbegin','<div class="record-empty"><strong>Aviso:</strong> Supabase todavía no reconoce la columna sector_club. El historial se muestra con los datos originales; recarga la caché del esquema antes de editar el sector.</div>');
    if(!rows.length){
    ...
    rows.forEach(row=>{
      const card=box.querySelector(`[data-season-card="${row.id}"]`),editor=card?.querySelector('[data-season-editor]'),save=card?.querySelector('[data-save-season]');if(!editor||!save)return;
      if(row.pago_club_declarado_at)card.insertAdjacentHTML('afterbegin',`<div class="season-club-status ${row.pago_club_verificado_at?'verified':'pending'}"><strong>${row.pago_club_verificado_at?'Pago al club verificado por la Peña':'Pago al club declarado, pendiente de verificar'}</strong><span>Declarado: ${esc(new Date(row.pago_club_declarado_at).toLocaleString('es-ES'))}${row.pago_club_verificado_at?` · Verificado: ${esc(new Date(row.pago_club_verificado_at).toLocaleString('es-ES'))}`:''}</span></div>`);
      card.querySelector('[data-edit-season]').textContent='Editar y revisar pago';
      save.insertAdjacentHTML('beforebegin',`<label>Tipo del club<input data-season-category value="${esc(row.categoria_club||'')}" placeholder="Adulto, joven o infantil"></label><label>Precio inicial (€)<input type="number" min="0" step="0.01" data-season-original value="${esc(row.precio_original??'')}"></label><label>Descuento del club (€)<input type="number" min="0" step="0.01" data-season-discount value="${esc(row.descuento_club??'')}"></label><label>Precio final del club (€)<input type="number" min="0" step="0.01" data-season-price value="${esc(row.precio_abono??'')}"></label><label>Quién gestiona la renovación<select data-season-management><option value="pena" ${row.gestion_abono==='pena'?'selected':''}>Renueva mediante la Peña</option><option value="club" ${row.gestion_abono==='club'?'selected':''}>Pagó directamente al club</option><option value="pendiente" ${row.gestion_abono==='pendiente'?'selected':''}>Pendiente de confirmar</option><option value="no_renueva" ${row.gestion_abono==='no_renueva'?'selected':''}>No renueva</option></select></label><label>Importe pagado registrado (€)<input type="number" min="0" step="0.01" data-season-paid value="${esc(row.importe_pagado??0)}"></label><label>Método de pago<select data-season-method><option value="">Sin confirmar</option><option value="transferencia" ${row.forma_pago==='transferencia'?'selected':''}>Transferencia</option><option value="tarjeta" ${row.forma_pago==='tarjeta'?'selected':''}>Tarjeta</option><option value="efectivo" ${row.forma_pago==='efectivo'?'selected':''}>Efectivo</option><option value="club" ${row.forma_pago==='club'?'selected':''}>Pago directo al club</option></select></label><label class="season-club-declaration"><input type="checkbox" data-season-club-declared ${row.pago_club_declarado_at?'checked':''}> El socio declara que pagó el abono directamente al club</label><label class="season-club-declaration"><input type="checkbox" data-season-club-verified ${row.pago_club_verificado_at?'checked':''}> La Peña ha comprobado la renovación en el club</label><button type="button" class="btn btn-dark-outline" data-confirm-transfer>Confirmar todo pagado por transferencia</button>`);
  ...
  async function syncSeasonFeeStatus(db,row,isPaid,method='transferencia'){
    ...
    if(!season?.id)return;
    const amount=Number(row.cuota_final||0),state=isPaid?(amount===0?'exenta':'pagada'):'pendiente';
    const {data:fee}=await db.from('cuotas_socios').select('id').eq('socio_id',memberId).eq('temporada_id',season.id).limit(1).maybeSingle();
    const values={importe:amount,estado:state,fecha_pago:isPaid?new Date().toISOString().slice(0,10):null,metodo_pago:isPaid?(method||'transferencia'):null,observaciones:isPaid?'Sincronizado con el pago conciliado de la temporada':'Pendiente de conciliación económica',updated_at:new Date().toISOString()};
    if(fee?.id)await db.from('cuotas_socios').update(values).eq('id',fee.id);else await db.from('cuotas_socios').insert({...values,socio_id:memberId,temporada_id:season.id});
  ...
  async function addPaymentButton(db,m){
    ...
    const feeResult=await db.from("cuotas_socios").select("id,estado,importe,temporada_id,temporadas(id,nombre,activa)").eq("socio_id",memberId).order("created_at",{ascending:false}).limit(20);
    if(feeResult.error)return;
    const fees=feeResult.data||[];
    let fee=fees.find(x=>String(x.estado||'').toLowerCase()==='pendiente')||fees.find(x=>x.temporadas?.activa)||fees[0];
    let season=fee?.temporadas||null;
    if(!season){
      const seasonResult=await db.from('temporadas').select('id,nombre,activa').order('activa',{ascending:false}).order('nombre',{ascending:false}).limit(1).maybeSingle();
    ...
    box.insertAdjacentHTML("afterend",`<div id="recordFeeAction" style="margin-top:14px;display:flex;flex-wrap:wrap;gap:10px;align-items:center"><button id="recordMarkPaid" type="button" class="btn btn-primary">Marcar cuota de la Peña como pagada · ${Number(fee.importe||0).toLocaleString("es-ES",{style:"currency",currency:"EUR"})}</button><button id="recordPaidClub" type="button" class="btn btn-dark-outline">Marcar: renueva el abono con el club</button><small style="display:block;flex-basis:100%;margin-top:2px">Temporada ${esc(season.nombre)}. La cuota de socio y la renovación del abono se registran por separado.</small></div>`);
    ...
    $("#recordPaidClub").onclick=async()=>{
      if(!confirm(`¿Confirmas que en ${season.nombre} este socio gestionó su abono directamente con el Málaga CF? El precio del abono quedará sin informar y la Peña solo controlará su cuota.`))return;
      const campaign=await db.from('campanas_abonados').select('id').eq('temporada',season.nombre).neq('tipo','piloto').order('updated_at',{ascending:false}).limit(1).maybeSingle();
      if(campaign.error)return window.FrenteNotify.error(campaign.error.message);
      if(campaign.data?.id){
        const existing=await db.from('campanas_registros').select('id').eq('campana_id',campaign.data.id).eq('socio_id',memberId).limit(1).maybeSingle();
        if(existing.error)return window.FrenteNotify.error(existing.error.message);
        const values={gestion_abono:'club',precio_original:null,precio_abono:null,descuento_club:null,cuota_final:Number(fee.importe||0),pago_club_declarado_at:new Date().toISOString(),forma_pago:null,observaciones:'Abono renovado directamente con el club. Precio desconocido para la Peña; solo se controla la cuota de socio.'};
        const saved=existing.data?.id?await db.from('campanas_registros').update(values).eq('id',existing.data.id):await db.from('campanas_registros').insert({...values,campana_id:campaign.data.id,socio_id:memberId,origen:'manual',estado:'pendiente_revision'});
        if(saved.error)return window.FrenteNotify.error(saved.error.message);
      }
      const memberSaved=await db.from('socios').update({gestion_abono_preferida:'club'}).eq('id',memberId);
      if(memberSaved.error)return window.FrenteNotify.error(memberSaved.error.message);
      window.FrenteNotify.success('Renovación directa con el club registrada. El precio del abono queda sin informar.');
      setTimeout(()=>location.reload(),500);
    };
  }
