(function(){
  ...
  async function homeMaterial(){
    ...
    function show(n){index=(n+items.length)%items.length;img.src=items[index].photo;img.alt=items[index].nombre;caption.textContent=items[index].mostrar_precio&&items[index].precio!=null?items[index].nombre+' · '+money(items[index].precio):items[index].nombre;qa('.fc408-carousel-dots button').forEach(function(dot,i){dot.classList.toggle('active',i===index)})}
