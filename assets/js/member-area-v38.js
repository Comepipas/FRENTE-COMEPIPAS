(() => {
 ...
 async function loadFamily(){
  ...
  }catch(err){console.warn('[Commit 38] Menores vinculados:',err.message||err);section.hidden=true;}
 }
 ...
 async function loadRealPayment(profile){
  ...
  try{
   ...
   const rows=(data||[]).filter(r=>{
    const season=String(r.campanas?.temporada||'').trim().toLowerCase();
  ...
  }catch(err){console.warn('[Commit 38] Pago real no disponible:',err.message||err);set('renewalStatus','No disponible');set('memberRenewalSummary','Importe no disponible');}
 }
