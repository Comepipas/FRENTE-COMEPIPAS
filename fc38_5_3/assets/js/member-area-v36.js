(() => {
 ...
 function openTab(tab){document.querySelectorAll('[data-member-tab]').forEach(x=>x.classList.toggle('active',x.dataset.memberTab===tab));document.querySelectorAll('[data-member-panel]').forEach(x=>x.classList.toggle('active',x.dataset.memberPanel===tab));document.getElementById('memberMoreSheet')?.setAttribute('hidden','');window.scrollTo({top:0,behavior:'smooth'});}async function logout(){await MemberAuth.signOut();location.href='socios.html';}
document.addEventListener('DOMContentLoaded',()=>{load();document.querySelectorAll('[data-member-tab]').forEach(b=>b.onclick=()=>openTab(b.dataset.memberTab));document.querySelectorAll('[data-open-member-tab]').forEach(b=>b.onclick=()=>openTab(b.dataset.openMemberTab));document.querySelectorAll('[data-member-tab-shortcut]').forEach(b=>b.onclick=()=>openTab(b.dataset.memberTabShortcut));document.getElementById('memberMoreButton')?.addEventListener('click',()=>document.getElementById('memberMoreSheet')?.removeAttribute('hidden'));document.querySelectorAll('[data-close-member-more]').forEach(b=>b.addEventListener('click',()=>document.getElementById('memberMoreSheet')?.setAttribute('hidden','')));document.getElementById('memberLogout')?.addEventListener('click',logout);document.getElementById('memberLogoutMore')?.addEventListener('click',logout);});
})();
